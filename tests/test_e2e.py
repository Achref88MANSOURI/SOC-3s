...
SAMPLE_PATH = Path(__file__).resolve().parent.parent / "alert-sample.json"


def _load_raw_so_event() -> dict:
    with open(SAMPLE_PATH) as f:
        data = json.load(f)
    return data[0]["body"]


def _build_raw_alert_from_sample() -> dict:
    """alert-sample.json is the RAW Security Onion webhook body n8n receives —
    upstream of n8n's Alert Builder node, not what agent-service actually gets.
    Convert it into the AlertWebhookPayload.raw_alert shape n8n actually sends
    (SOC-3s-ARCHITECTURE-v2.md §3: type/source/sourceRef/title/description/
    severity/tlp/pap/date/tags/observables), using real fields pulled from the
    sample (rule name/uuid, hostname, command line, process hash)."""
    body = _load_raw_so_event()
    rule = body["rule"]
    host = body["event_data"]["host"]
    process = body["event_data"]["process"]

    ips = [ip for ip in host.get("ip", []) if ip not in ("127.0.0.1", "::1") and not ip.startswith("fe80")]
    host_ip = ips[0] if ips else "0.0.0.0"

    description = (
        "Detection engine: sigma\n"
        f"Rule: {rule['name']} ({rule['uuid']})\n"
        f"Host: {host['name']} ({host_ip})\n"
        f"Command line: {process['command_line']}"
    )
    ...
    return {
        ...
        "sourceRef": body["_id"],
        "title": f"[HIGH] {rule['name']} - {host['name']}",
        ...
        "observables": [
            {"dataType": "hash", "data": process["hash"]["sha256"], "ioc": True, "tags": ["sha256"]},
        ],
    }
...
def test_triage_end_to_end_new_alert_no_500(
    ...
    """Full /triage round trip with the real alert-sample.json data, all external
    calls mocked: TheHive fetch (Gate 0/perceive's pre-fetch), the LLM behind
    Agent 1 (perceive) and Agent 2 (investigate) ReAct loops, and Agent 3's direct
    LLM call. Qdrant/ES/iTop/Cortex are only reachable through the ReAct agents'
    tool-calling loop, which is mocked wholesale here, so they're covered
    transitively — no live service of any kind is required."""
    ...
    payload = {
        ...
        "raw_alert": _build_raw_alert_from_sample(),
